      ...
      return (
            <nav className="bg-primary w-[307px] h-screen flex flex-col items-center justify-between py-8 overflow-hidden">
                  ...
                  <div className="logo">
                        ...
                        {navItems.map((nav) => (
                              <Link href={nav.href} key={nav.label} className="text-secondary w-[200px] rounded-2xl text-center text-base bg-[#AADC8D] p-2">
                                    {nav.label}
                              </Link>
                        ))}
                  </div>
                  <div>
                        <Image src="/image/background-sidebar.svg" alt="background sidebar" width={200} height={150} className="w-full h-auto object-contain" />
                  </div>
            </nav>
      )
}